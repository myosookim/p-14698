...
public class ProductService {
    ...
    @Autowired
    private EmbeddingModel embeddingModel;  //Spring AI의 임베딩 모델 (키워드를 벡터로 변환)
    @Autowired
    ...
    public Product create(String name, List<String> keywords) {
        ...
        doc.setEmbedding(calculateAverage(embeddings));  // calculateAverage(~) : 여러 키워드 벡터의 평균을 계산하여 상품 벡터 생성
    ...
    // Elasticsearch의 K-Nearest Neighbors 검색 API
    ...
    public List<ProductDocument> knnSearchByVector(float[] queryVector, String field, int k, int numCandidates) {
        ...
        KnnSearch knnSearch = KnnSearch.of(knn -> knn
                ...
                .field(field)
                .k(k)
                .numCandidates(numCandidates) // numCandidates = 검색 후보 수 (정확도와 성능 트레이드오프)
        );
